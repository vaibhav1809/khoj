...
def completion_with_backoff(**kwargs):
    ...
    if not "anthropic_api_key" in kwargs:
        kwargs["anthropic_api_key"] = os.getenv("ANTHROPIC_API_KEY")
    llm = ChatAnthropic(**kwargs, request_timeout=20, max_retries=1)
...
def chat_completion_with_backoff(
    messages, compiled_references, model_name, temperature, anthropic_api_key=None, completion_func=None
):
    g = ThreadedGenerator(compiled_references, completion_func=completion_func)
    t = Thread(target=llm_thread, args=(g, messages, model_name, temperature, anthropic_api_key))
    t.start()
    return g


def llm_thread(g, messages, model_name, temperature, anthropic_api_key=None):
